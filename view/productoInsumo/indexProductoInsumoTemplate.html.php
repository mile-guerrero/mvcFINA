...
<div class="container container-fluid" id="cuerpo">
  <header id="">
    ...
    <a class="btn btn-danger btn-xs" href="<?php echo routing::getInstance()->getUrlWeb('productoInsumo', 'insertProductoInsumo') ?>"><?php echo i18n::__('nuevo') ?></a> <a href="javascript:eliminarMasivo()" class="btn btn-danger btn-xs" id="btnDeleteMasivo"><?php echo i18n::__('eliminar en masa') ?></a> <button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#myModalFiltres"><?php echo i18n::__('filtros') ?></button>  <a href="<?php echo routing::getInstance()->getUrlWeb('productoInsumo', 'indexProductoInsumo') ?>" class="btn btn-default btn-xs" ><?php echo i18n::__('eFiltros') ?></a> <a class="btn btn-danger btn-xs" href="<?php echo routing::getInstance()->getUrlWeb('productoInsumo', 'reportProductoInsumo') ?>"><?php echo i18n::__('informe') ?></a>            
    </ul> 
    ...
    <div class="modal fade" id="myModalFiltres" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form class="form-horizontal" id="filterForm" role="form" action="<?php echo routing::getInstance()->getUrlWeb('productoInsumo', 'indexProductoInsumo') ?>" method="POST">
              ...
              <div class="form-group">
                <label for="filterIva" class="col-sm-2 control-label"><?php echo i18n::__('iva') ?></label>
                <div class="col-sm-10">
                  <input type="text" class="form-control" id="filterIva" name="filter[iva]" placeholder="<?php echo i18n::__('iva') ?>">
                </div>
              </div>
     ...
     <form class="form-signin" id="frmDeleteAll" action="<?php echo routing::getInstance()->getUrlWeb('productoInsumo', 'deleteSelectProductoInsumo') ?>" method="POST">        
                ...
                <tr>
                 ...
                 <th>
                    <?php echo $key->$des ?>
                      ...
                      <th>
                      <a class="btn btn-danger btn-xs" href="<?php echo routing::getInstance()->getUrlWeb('productoInsumo', 'verProductoInsumo', array(productoInsumoTableClass::ID => $key->$id)) ?>" ><?php echo i18n::__('ver') ?></a> - <a class="btn btn-danger btn-xs" href="<?php echo routing::getInstance()->getUrlWeb('productoInsumo', 'editProductoInsumo', array(productoInsumoTableClass::ID => $key->$id)) ?>"><?php echo i18n::__('modificar') ?></a>- <a data-toggle="modal" data-target="#myModalDelete<?php echo $key->$id ?>" class="btn btn-danger btn-xs"><?php echo i18n::__('eliminar') ?></a>
                      </th>
              </tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-danger" onclick="eliminar(<?php echo $key->$id ?>,'<?php echo productoInsumoTableClass::getNameField(productoInsumoTableClass::ID, true) ?>','<?php echo routing::getInstance()->getUrlWeb('productoInsumo', 'deleteProductoInsumo') ?>')"><?php echo i18n::__('eliminar') ?></button>
      </div>
    </div>
  </div>
</div>                                            
      ...
      <div class="text-right">
        Pagina <select id="slqPaginador" onchange="paginador(this, '<?php echo routing::getInstance()->getUrlWeb('productoInsumo', 'indexProductoInsumo')?>')">
         <?php for($x = 1; $x <= $cntPages; $x++):?>
           <option <?php echo (isset($page) and $page == $x) ? 'selected': '' ?> value="<?php echo $x ?>"><?php echo $x ?></option>
          <?php endfor;?>
        </select> <?php echo i18n::__('de') ?> <?php echo $cntPages ?>
      </div>
      <form id="frmDelete" action="<?php echo routing::getInstance()->getUrlWeb('produtoInsumo', 'deleteProductoInsumo') ?>" method="POST">
       <input type="hidden" id="idDelete" name="<?php echo productoInsumoTableClass::getNameField(produtoInsumoTableClass::ID, true) ?>">
    </form>
