...
<label for="<?php echo maquinaTableClass::getNameField(maquinaTableClass::ORIGEN_MAQUINA, true) ?>" class="col-sm-2"> <?php echo i18n::__('origenM') ?>:  </label>     
        <div class="col-lg-5">
         <input class="form-control" value="<?php echo (session::getInstance()->hasFlash('inputOrigen') or request::getInstance()->hasPost(maquinaTableClass::getNameField(maquinaTableClass::ORIGEN_MAQUINA, true))) ? request::getInstance()->getPost(maquinaTableClass::getNameField(maquinaTableClass::ORIGEN_MAQUINA, true)) : ((isset($objMaquina[0])) ? $objMaquina[0]->$origen_id : '') ?>" type="text" name="<?php echo maquinaTableClass::getNameField(maquinaTableClass::ORIGEN_MAQUINA, true) ?>" placeholder="<?php echo i18n::__('origenM') ?>" required>
       </div>
