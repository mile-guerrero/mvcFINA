  ...
  class securityHookClass implements hookInterface {
    ...
    private static function verifyCookieRememberMe() {
      ...
      if (request::getInstance()->hasCookie(config::getCookieNameRememberMe())) {
        ...
        if (($objUsuarios = \recordarMeTableClass::getUserAndPassword(request::getInstance()->getServer('REMOTE_ADDR'), request::getInstance()->getCookie(config::getCookieNameRememberMe()))) !== false) {
    ...
    public static function login($objUsuarios) {
      session::getInstance()->setUserAuthenticate(true);
      session::getInstance()->setUserName($objUsuarios[0]->usuario);
      session::getInstance()->setUserId($objUsuarios[0]->id_usuario);
      foreach ($objUsuarios as $usuario) {
        session::getInstance()->setCredential($usuario->credencial);
      }
      \usuarioTableClass::setRegisterLastLoginAt($objUsuarios[0]->id_usuario);
      return true;
    }
